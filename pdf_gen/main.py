...
class PDFCreate:
    ...
    def part_b_sub_table(self):
        ...
        headers = ["Mode", "Vehicle / Trans\nDoc No & Dt.", "From", "Entered Date", "Entered By", "CEWB No.\n(If any)",
                   "Multi Veh.Info\n(If any)"]
        values = ["Road", "262029821 & 02/04/2018", p1, self.ewb_date.strftime("%d/%m/%Y %I:%M %p"), "29AAACT1507C1ZT",
                  "-",
                  "-"]
        new_headers = list()
        new_values = list()
    ...
    def bar_code(self):
        ...
        fp = io.BytesIO()
        barcode_image.write(fp, {"module_width":0.09, "module_height":4, "font_size": 3, "quiet_zone": 1})
    ...
    def get_e_way_table(self):
        ...
        table_data = [
            [self._get_as_label("E-Way Bill No:"), self._get_as_big_bold(self.row["EWB No"])],
    ...
    def build(self, index):
        ...
        file_name = f"{index}.{self.row['EWB No']}.pdf"
        doc = SimpleDocTemplate(file_name, leftMargin=0.25 * inch, rightMargin=0.25 * inch, topMargin=0.25 * inch,
